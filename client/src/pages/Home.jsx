...
const Home = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:5000/api/chat/conversations/${activeConversation._id}/read`,
      {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      },
  ...
  return (
    <div className="flex h-screen w-full bg-background text-foreground overflow-hidden">
      {/* Sidebar Panel */}
      <div className="w-80 h-full flex flex-col shrink-0 bg-card border-r border-border">
        <Sidebar
      ...
      {/* Main Chat Panel with smooth cross-fade animation when switching channels */}
      <main className="flex-1 h-full min-w-0 bg-background relative">
        <AnimatePresence mode="wait">
          <motion.div
            key={activeConversation?._id || "empty-state"}
